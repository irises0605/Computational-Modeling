...
H_births = @(P_population, H_population) ...
           H_birth_fraction * P_population * H_population;

Y_deaths = @(Y_population, P_population) ...
           Y_death_constant * P_population * Y_population;
P_deaths = @(P_population, H_population) ...
           P_death_constant * H_population * P_population;
...
for i = 1:N-1
    ...
    Y_pop(i+1) = Y_pop(i) + (Y_births(Y_pop(i))...
                 - Y_deaths(Y_pop(i), P_pop(i)))*dt;
    P_pop(i+1) = P_pop(i) + (P_births(Y_pop(i), P_pop(i))...
                 - P_deaths(P_pop(i), H_pop(i)))*dt;
    H_pop(i+1) = H_pop(i) + (H_births(P_pop(i), H_pop(i))...
                 - H_deaths(H_pop(i)))*dt;
end
...
title('Tuna, Shark & Human Population vs. Time')
xlabel('Time (month)')
