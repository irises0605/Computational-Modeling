%{
    ...
    Model: Hodgkin-Huxley Model with
           voltage-sensitive sodium, potassium channels
...
% h: initial active sodium chan nels inactivation probability
V(1) = -65;
n(1) = 0.317;
m(1) = 0.05;
h(1) = 0.6;
...
% dVdt_close: leakage flow ONLY
% dVdt_Na_open: leakage and Na flow
% dVdt_K_open: leakage and K flow
% dVdt_open: leakage, Na and K flow
dVdt_close = @(t, V, n, m, h, I_i) ...
              (I_i - calc_I_L(V))/C_m;
dVdt_Na_open = @(t, V, n, m, h, I_i) ...
                (I_i - calc_I_Na(m, h, V) - calc_I_L(V))/C_m;
dVdt_K_open = @(t, V, n, m, h, I_i) ...
               (I_i - calc_I_K(n, V) - calc_I_L(V))/C_m;
dVdt_open = @(t, V, n, m, h, I_i) ...
             (I_i - calc_I_Na(m, h, V) - calc_I_K(n, V) - calc_I_L(V))/C_m;
