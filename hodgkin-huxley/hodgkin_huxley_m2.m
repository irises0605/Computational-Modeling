%{
    ...
    Model: Hodgkin-Huxley Model with
           ...
           leakage channels, and Na-K pump
...
% V_K: potassium displacement from equilibrium potential
% V_Na: sodium displacement from equilibrium potential
% V_L: leakage displacement from equilibrium potential
V_rest = -65;
V_K = -77;
V_Na = 50;
V_L = -54.4;
...
% dVdt_close: leakage and constant Na-K pump flows
% dVdt_Na_open: leakage, constant Na-K pump, and Na flow
% dVdt_K_open: leakage, constant Na-K pump, and K flow
% dVdt_open: leakage, constant Na-K pump, Na and K flow
dVdt_close = @(t, V, n, m, h, I_i) ...
              (I_i - calc_I_L(V) + I_P)/C_m;
dVdt_Na_open = @(t, V, n, m, h, I_i) ...
                (I_i - calc_I_Na(m,h,V) - calc_I_L(V) + I_P)/C_m;
dVdt_K_open = @(t, V, n, m, h, I_i) ...
               (I_i - calc_I_K(n,V) - calc_I_L(V) + I_P)/C_m;
dVdt_open = @(t, V, n, m, h, I_i) ...
             (I_i - calc_I_Na(m,h,V) - calc_I_K(n,V) - calc_I_L(V) + I_P)/C_m;
...
for i = 1:N-1
    ...
    dV2 = dt * dVdt(t(i)+0.5*dt, V(i)+0.5*dV1, n(i)+0.5*dn1, m(i)+0.5*dm1, h(i)+0.5*dh1,I(i));
    ...
    dV3 = dt * dVdt(t(i)+0.5*dt, V(i)+0.5*dV2, n(i)+0.5*dn2, m(i)+0.5*dm2, h(i)+0.5*dh2,I(i));

    dn4 = dt * dndt(t(i)+dt, n(i)+dn3, V(i)+dV3);
    dm4 = dt * dmdt(t(i)+dt, m(i)+dm3, V(i)+dV3);
    dh4 = dt * dhdt(t(i)+dt, h(i)+dh3, V(i)+dV3);
    dV4 = dt * dVdt(t(i)+dt, V(i)+dV3, n(i)+dn3, m(i)+dm3, h(i)+dh3, I(i));
